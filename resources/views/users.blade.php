            ...
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    ...
                    <tbody>
                        @php
                        $users = [
                            ['Admin Barangay',  'admin.brgy',   'admin',     'Feb 25, 2025 · Now',    'online',  '#1a56db','#dbeafe'],
                            ['Pedro B. Garcia', 'pedro.garcia', 'secretary', 'Feb 25, 2025 · 10:32AM','online',  '#7c3aed','#ede9fe'],
                            ['Nena D. Torres',  'nena.torres',  'secretary', 'Feb 25, 2025 · 9:10AM', 'online',  '#7c3aed','#ede9fe'],
                            ['Maria L. Santos', 'maria.santos', 'committee', 'Feb 24, 2025 · 4:18PM', 'offline', '#16a34a','#dcfce7'],
                            ['Juan P. Cruz',    'juan.cruz',    'committee', 'Feb 24, 2025 · 2:00PM', 'offline', '#16a34a','#dcfce7'],
                            ['Ana B. Reyes',    'ana.reyes',    'committee', 'Feb 23, 2025 · 11:45AM','offline', '#16a34a','#dcfce7'],
                            ['Rosa T. Aquino',  'rosa.aquino',  'viewer',    'Feb 22, 2025 · 3:22PM', 'offline', '#64748b','#f1f5f9'],
                        ];
                        $roleMap = ['admin'=>['badge-admin','Admin'],'secretary'=>['badge-secretary','Secretary'],'committee'=>['badge-committee','Committee'],'viewer'=>['badge-viewer','Viewer']];
                        $statMap = ['online'=>['badge-online','Online'],'offline'=>['badge-offline','Offline']];
                        @endphp
                        @foreach($users as $u)
                        @php [$name,$uname,$role,$login,$stat,$ac,$abg] = $u; [$rbc,$rbl] = $roleMap[$role]; [$sbc,$sbl] = $statMap[$stat];
                             $initials = collect(explode(' ',$name))->map(fn($w)=>$w[0])->take(2)->implode(''); @endphp
                        <tr>
                            <td>
                                <div class="d-flex align-items-center gap-2">
                                    <div class="user-avatar" style="background:{{ $abg }};color:{{ $ac }};">{{ $initials }}</div>
                                    <span style="font-weight:600;font-size:13px;">{{ $name }}</span>
                                </div>
                            </td>
                            <td style="font-family:'DM Mono',monospace;font-size:12px;color:#64748b;">{{ $uname }}</td>
                            <td><span class="status-badge {{ $rbc }}">{{ $rbl }}</span></td>
                            <td style="font-size:12px;color:#64748b;">{{ $login }}</td>
